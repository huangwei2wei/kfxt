...
class QuestionAction extends SafetyAction{
	...
	public function index(){
		$this->assign("game_type",$this->getSysConfig("game_type"));	
		//显示所有问题类型
	...
	public function save(){
		...
		$game_type = $this->getSysConfig("game_type");
		$UwanGameId = $game_type[$_GET['gameid']]['UwanGameId'];
		$sendArr=$this->_getUserResult($_POST['user_account'],$_POST['game_server_id'],$UwanGameId);
		if (!$_POST['game_server_id']){
			if ($_POST['game_id']==1)$_POST['game_server_id']=100;
			if ($_POST['game_id']==2)$_POST['game_server_id']=200;
			if ($_POST['game_id']==5)$_POST['game_server_id']=20;
		...
		if ($backParams->status==1){
			...
			$this->success("您的问题已受理，此问题的等候队列：第 ".$this->_getQueue($_POST['game_type'],$backParams->data->room_id)." 位，请耐心等侍。");
		}else {
