	...
	<img src="/Public/front/default/images/1.jpg" />
            ...
            <?php if(is_array($faqKind)): $i = 0; $__LIST__ = $faqKind;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><option  value="<?php echo ($vo["Id"]); ?>" <?php if($vo["Id"] == $gameTypeId): ?>selected="selected"<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?> 
					...
					<div class="servicerli">
						<ul>
                            <?php if(is_array($faqList)): foreach($faqList as $key=>$vo): ?><li><a href="<?php echo U("/Faq/show/game_type_id/$gameTypeId/Id/$vo[Id]");?>"><?php echo (stripcslashes($vo["question"])); ?></a></li><?php endforeach; endif; ?>
                            
                            <?php if(!isset($faqList)): ?>抱歉，没有找到与"<?php echo ($keyword); ?>" 相关内容，欢迎<a href="<?php echo U('Question/index');?>">提交问题</a>给我们<?php endif; ?>
						</ul>
					</div>
