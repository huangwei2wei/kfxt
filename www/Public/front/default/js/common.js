...
function loadQuestionTypes(gameId){
	...
	if(gameId == 1){
		$('#other_game_left').hide();
		$('#bto_special_left').show();
	}else{
		$('#other_game_left').show();
		$('#bto_special_left').hide();
	}
...
function addUpFile(){
	if(fileCount<3){
		var obj = $('#upfile_list');
		var temp='<div id="file_'+fileI+'"><input type="file" style="width: 212px; height:22px;margin-left:115px;margin-top:5px" name="file_upload[];"/> <a style="font-size:12px;margin-left:8px" href="javascript:delUpFile(\'file_'+fileI+'\')" >删除</a></div>';//
		fileI++;
		fileCount++;
		obj.append(temp);
	}else{
		...
		success: function(result){
			if(result.status==1 && result.data){
				...
				if(len>0){
					...
					ls.empty();
					for(var i=0;i<len;i++){
						ls.append("<a target='_blank' title='"+result.data[i].question+"' href='/index.php?s=/Faq/show/Id/"+result.data[i].Id+"'>"+subStr(result.data[i].question,32)+"</a><br />");
					}
				}
			}
		}
