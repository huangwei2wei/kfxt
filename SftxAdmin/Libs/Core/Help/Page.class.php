...
class Help_Page {
	...
	function input(){
		...
		$return='<b>['.$this->nowindex.'/'.$this->totalpage.']</b> <input type="text" size="10" class="text" id="pageIndex" value="'.$this->nowindex.'" /> <input class="btn-blue" onclick="pageInput()" type="button" id="pageButton" url="'.$this->url.'" value="确定" />';
		return $return;
	}
	...
	function show($mode = 1) {
		switch ($mode) {
			case '1' :
				...
				$this->first_page = ' 首页 ';
				$this->last_page = ' 尾页 ';
				return  $this->first_page () . $this->pre_page () . $this->nowbar () . $this->next_page () . $this->last_page ()  . $this->input();
				break;
			case '2' :
				$this->next_page = '下一页';
				$this->pre_page = '上一页';
