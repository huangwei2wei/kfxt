...
class _PHPRPC_Client {
    ...
    function _PHPRPC_Client($serverURL = '') {
        ...
        //register_shutdown_function(array(&$this, "_disconnect"));       //add by kofboy
        $this->_proxy = NULL;
        $this->_timeout = 30;
    ...
    function useService($serverURL, $username = NULL, $password = NULL) {
        ...
        $this->_keep_alive = false;	//由true更改为false
        $this->_server = array();
        $this->_key = NULL;
        $this->_keylen = 128;
        $this->_encryptMode = 0;
    ...
    function setPrivateKey($key)       //add by kofboy
    {
        $this->_key = $key;
        $this->_encryptMode = 3;
    ...
    function _connect() {
        if (is_null($this->_proxy)) {
            ...
            $this->_socket = @pfsockopen($host, $this->_server['port'], $errno, $errstr, $this->_timeout);  //add by kofboy
        }
        else {
