...
class Model_ReplyQulity extends Model {
	...
	public function getStatsNum($gameTypeId,$operatorIds,$date){
		...
		$dataList=$this->select("select * from {$this->tName()} where game_type_id={$gameTypeId} and operator_id in ({$operatorIds}) and create_time between {$date['first']} and {$date['last']}");
		if ($dataList){
			$returnArr=array();
	...
	public function getOperatorStatsNum($gameTypeId,$operatorIds,$date){
		$operatorIds=implode(',',$operatorIds);
		$dataList=$this->select("select * from {$this->tName()} where game_type_id={$gameTypeId} and operator_id in ({$operatorIds}) and create_time between {$date['start']} and {$date['end']}");
		if ($dataList){
			$returnArr=array();
			foreach ($dataList as $value){
				$returnArr[date('Ymd',$value['create_time'])]['option_num'][$value['option_id']]++;
