...
class Help_Page {
	 ...
	 */
	function Help_Page($array) {
		if (is_array ( $array )) {
			if (! array_key_exists ( 'total', $array ))
				$this->error ( __FUNCTION__, 'need a param of total' );
			$total = intval ( $array ['total'] );
			$this->total = $total;	//Add by youxingyaun
			$this->showPageSize = $perpage = (array_key_exists ( 'perpage', $array )) ? intval ( $array ['perpage'] ) : 10;
			$nowindex = (array_key_exists ( 'nowindex', $array )) ? intval ( $array ['nowindex'] ) : '';
			$url = (array_key_exists ( 'url', $array )) ? $array ['url'] : '';
			$this->page_name=(array_key_exists('pagename',$array))?$array['pagename']:'page';//新增PAGEGET值 
		} else {
			$total = $array;
			$perpage = 10;
			$nowindex = '';
	 ...
	 */
	function select() {
		$return = '<select name="PB_Page_Select" onchange="location.href=$(this).val()" >';
	...
	function input(){
		if (intval($this->nowindex)<1)$this->nowindex=1;
		if (intval($this->nowindex)>$this->totalpage)$this->nowindex=$this->totalpage;
		
		$return='<b>['.$this->nowindex.'/'.$this->totalpage.']</b> <input type="text" size="10" class="text" id="pageIndex" value="'.$this->nowindex.'" /> <input class="btn-blue" onclick="pageInput()" type="button" id="pageButton" url="'.$this->url.'" value="Go" />';
		return $return;
	}
	...
	function show($mode = 1) {
		switch ($mode) {
			case '1' :
				...
				$this->last_page = ' Last ';
				return  $this->first_page () . $this->pre_page () . $this->nowbar () . $this->next_page () . $this->last_page () . $this->total() . $this->input();
				break;
			case '2' :
				$this->next_page = '下一页';
				$this->pre_page = '上一页';
		...
		if (! empty ( $url )) {
			...
			if (empty ( $_SERVER ['QUERY_STRING'] )) {
				...
				if (stristr ( $_SERVER ['QUERY_STRING'], $this->page_name . '=' )) {
					//地址存在页面参数
// 					echo $this->page_name . '=' . $this->nowindex;
					$this->url = str_replace ( '&'.$this->page_name . '=' . $this->nowindex, '', $_SERVER ['REQUEST_URI'] );
					$this->url = str_replace ( 'pageSize=' . $this->showPageSize, '', $this->url );
	...
	function _get_url($pageno = 1) {
		return $this->url . $pageno.'&pageSize='.$this->showPageSize;
	}
