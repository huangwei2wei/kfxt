...
abstract class Model extends Base {
	 ...
	 * @return array
	 */
	public static function getTtwoArrConvertOneArr($dataBase,$key='Id',$value,$prepend=NULL) {
...
//		$id=abs(intval($id));
	 ...
	 */
	public function adds($keyvalue,$table=null,$isExcludeEmpty = false){
		if (! $table)
			$table = $this->tName ();
		...
		foreach ($keyvalue as $list){
			if($isExcludeEmpty){	//是否排除空字段
				$list = array_filter($list);
				$key = implode ( ',', array_keys ( $list ) );
			}
	...
	public function isTableExists($tableName){
		$tables=$this->_getInstance()->select("SHOW TABLES LIKE '{$tableName}'");
	 ...
	 * 求group by 语句的总数
	 */
	public function countGroupBy($conditions=NULL,$FiledGroupBy=NULL){
		if($FiledGroupBy){
			$sql = "select count(DISTINCT {$FiledGroupBy}) as total_num from {$this->tName()}";
			if ($conditions!=null)$sql.=" where {$conditions} ";
		}else{
			$sql = "select count(*) from {$this->tName()}";
			if ($conditions!=null)$sql.=" where {$conditions} ";
			$sql = "select count(*) as total_num from ({$sql}) tb_gb";
		}
		$count = $this->select ( $sql );
		return $count [0] ['total_num'];
	}
