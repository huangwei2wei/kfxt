...
class ActionGame_MasterTools_Silence_LuanShi extends Action_ActionBase{
	const INGAME = 'inGame';
	const LOGTYPE = 2;
	...
	private function _dataInGame($UrlAppend=null,$get=null,$post=null){
		...
		$dataList = array();
		if(is_array($data['data']['playerList'])){
			foreach($data['data']['playerList'] as $key => $sub){
				$dataList[$key]['game_user_id'] = $sub['playerId'];
				$dataList[$key]['game_user_account'] = $sub['pname'];
				$dataList[$key]['game_user_nickname'] = $sub['prolename'];
				$dataList[$key]['endTime'] = $sub['donttalkEndTime']==1?"--已手动解封": date('Y-m-d H:i:s',$sub['donttalkEndTime']);
				$dataList[$key]['URL_del'] = $this->_urlSilenceDel($sub['playerId'],$sub['pname'],$sub['prolename']);
			}
		}
