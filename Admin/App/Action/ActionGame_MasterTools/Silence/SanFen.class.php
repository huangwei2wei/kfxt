...
class ActionGame_MasterTools_Silence_SanFen extends Action_ActionBase{
	const INGAME = 'inGame';
	const LOGTYPE = 2;
...
// 		print_R($data);
		...
		if(is_array($list)){
			foreach($list as $key => &$sub){
				...
				$sub['endTime'] = date('Y-m-d H:i:s',$sub['endTime']/1000);
// 				$sub['startTime'] = date('Y-m-d H:i:s',$sub['startTime']);
				$sub['URL_del'] = $this->_urlSilenceDel($sub['playerId'],$sub['accountName'],$sub['playerName']);
			}
