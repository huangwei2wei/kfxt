...
class ActionGame_MasterTools_Silence_DaoJian extends Action_ActionBase{
	const INGAME = 'inGame';
	const LOGTYPE = 2;
		...
		if(is_array($list)){
			foreach($list as $key => $sub){
				if($sub['forbitChat']!= null){
					...
					$dataList[$key]['endTime'] = date('Y-m-d H:i:s',round($sub['forbitChat']/1000 ));
					//$dataList[$key]['startTime'] = date('Y-m-d H:i:s',$sub['startTime']);
					$dataList[$key]['URL_del'] = $this->_urlSilenceDel($sub['playerId'],$sub['userName'],$sub['playerName']);
				}
			}
		}
