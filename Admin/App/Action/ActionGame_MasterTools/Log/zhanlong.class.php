...
class ActionGame_MasterTools_Log_zhanlong extends Action_ActionBase{
	...
	public function main($UrlAppend=NULL,$get=NULL,$post=NULL){
		...
		$this->_assign['moneytype']=$this->_gameObject->getMoney();
...
//			twSelect($sql,$url,$user="",$pwd="",$is_ac=false);
		...
		foreach($data as $k=>&$v){
			$now=0;
			$r=array();
			foreach($RoleLevel as $rk=>$rv){
				if($now<=abs(strtotime($rv["dtEventTime"])-strtotime($v["dtEventTime"]))){
					$now = abs(strtotime($rv["dtEventTime"])-strtotime($v["dtEventTime"]));
					$r = $rv;
				}
			}
			$v["vRoleName"]		=	$r["vRoleName"];
			$v["iRoleLevel"]	=	$r["iRoleLevel"];
			$v["iRoleExp"]		=	$r["iRoleExp"];
			$v["iMoney"]		=	$r["iMoney"];
			$v["iUin"]			=	$r["iUin"];
			$v["iJobId"]		=	$Job[$r["iJobId"]];
			$v["iGamePoints"]	=	$r["iGamePoints"];
			$v["iFightPower"]	=	$r["iFightPower"];
			$v["iYuanLi"]		=	$r["iYuanLi"];
