...
class ActionGame_MasterTools_SilenceAdd_SanFen extends Action_ActionBase{
	...
	public function getPostData($post=null){
		...
		$this->_endTime = trim($_POST['endTime'])?date("Y-m-d",strtotime(trim($_POST['endTime']))):'';
...
//  	print_r($data);print_r($postData);print_r($getData);exit;
			if($data['status'] == 1){
				foreach($data['data'] as $sub){
					$this->_playersData[]=array(
							'playerId'=>trim($sub['uid']),
							'playerAccount'=>trim($sub['userName']),
							'playerNickname'=>trim($sub['nackName']),
					);
				}
