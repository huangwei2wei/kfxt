...
class ActionGame_MasterTools_PlayerLog_HuanJL extends Action_ActionBase{
	...
	public function getPostData($post=NULL){
		$postData = array(
			...
			'NickName' => trim($_GET['name']),
			'keywords'=>trim($_GET['keywords']),
				
				
			'SerialType'=>intval($_GET['rootid']),
			'Oper'=>intval($_GET['typeid']),
...
// 		print_r($data);exit;
		...
		if($data['status'] == '1'){
			...
			foreach($data['data']['list'] as &$sub){
				...
				$sub['playerType'] = $playerLogTypes[$sub['SerialType']]['rootTypeName'].'-->'.$playerLogTypes[$sub['SerialType']]['subTypeList'][$sub['Oper']]['subTypeName'];
