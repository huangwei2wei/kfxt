...
class ActionGame_MasterTools_PlayerRoleList_zhanlong extends Action_ActionBase{
	...
	public function main($UrlAppend=NULL,$get=NULL,$post=NULL){
		...
		if($this->_isAjax()){
			$sendData["Level"] 		= intval($_POST["Level"]);
			$sendData["CreateTime"] = $_POST["CreateTime"];
			$accountCount = $this->getResult('QueryPlayer/PlayerCount',array_merge($getData,$sendData));
			echo $accountCount["PlayerCount"];
			die();
		}
		...
		$account = $this->getResult('QueryPlayer/AccountCount',$getData);

		$serverList = $this->_getGlobalData('server/server_list_'.$this->_gameObject->_gameId);
		$this->_assign['_WorldID'] = $serverList[$_REQUEST['server_id']]['ordinal'];
		$this->_assign['Order'] = intval($_GET['Order']);
		$this->_assign['AccountCount'] = $account['AccountCount'];
		if(!$_REQUEST['sbm']){
			return $this->_assign;
		}
		...
		if($data['Result']===0){
			...
			if($data['PlayerList']){
				...
				$this->_assign['dataList'] = $data['PlayerList'];
