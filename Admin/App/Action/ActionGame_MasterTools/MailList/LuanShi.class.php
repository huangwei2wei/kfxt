...
class ActionGame_MasterTools_MailList_LuanShi extends Action_ActionBase{
...
//		$this->_assign['URL_EquipmentDel'] = Tools::url(CONTROL,'EquipmentDel',array('zp'=>PACKAGE,'__game_id'=>$this->_gameObject->_gameId,'server_id'=>$_REQUEST['server_id']));
	...
	public function getPostData($post=null){
		$postData = array();
		$postData['toPlayerId'] = trim($_GET['toPlayerId']);
		$postData['fromPlayerId'] = trim($_GET['fromPlayerId']);
		...
		if(is_array($data) && $data['status']==1 ){
			if($data['data']['list']){
				foreach($data['data']['list'] as &$_msg){
					...
					$_msg['time'] = date('Y-m-d H:i:s',$_msg['logTime']);
				}
