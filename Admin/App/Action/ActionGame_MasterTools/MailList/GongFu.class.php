...
class ActionGame_MasterTools_MailList_GongFu extends Action_ActionBase{
...
//		$this->_assign['URL_EquipmentDel'] = Tools::url(CONTROL,'EquipmentDel',array('zp'=>PACKAGE,'__game_id'=>$this->_gameObject->_gameId,'server_id'=>$_REQUEST['server_id']));
	...
	public function getPostData($post=null){
		$postData = array();
		$postData['playerType'] = urlencode(trim($_GET['playerType']));
		$postData['player'] = urlencode(trim($_GET['player']));
		$postData['actionType'] = trim($_GET['mailType']);
		$postData['haveAtt'] = trim($_GET['haveAtt']);
		$postData['gotAtt'] = trim($_GET['gotAtt']);
...
// 		print_r($data);
		...
		if($data['status']==1){
			if($data['data']['list']){
				...
				$helpPage=new Help_Page(array('total'=>$data['data']['counts'],'perpage'=>PAGE_SIZE));
