...
class ActionGame_MasterTools_PlayerPartnerInfo_MingXing extends Action_ActionBase{
	...
	public function getPostData($post=null){
		...
		$postData['actorId'] = trim($_GET['actorId']);
		$postData = array_filter($postData);
		if(!($postData['nickName'].$postData['openId']) || !$postData['actorId']){
			$this->jump('参数缺少',-1);
		}
		return $postData;
	}
