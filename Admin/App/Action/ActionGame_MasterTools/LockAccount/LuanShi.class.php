...
class ActionGame_MasterTools_LockAccount_LuanShi extends Action_ActionBase{
	const INGAME = 'inGame';
	const LOGTYPE = 1;
	...
	private function _dataInGame($UrlAppend=null,$get=null,$post=null){
		...
		$getData = array_merge($getData,
				array('page'=>max(1,intval($_GET['page'])))
				);
		$data = $this->getResult($UrlAppend,$getData,null);
		$list =$data['data']['playerList'];
		if(is_array($list)){
			foreach($list as $key => &$sub){
				$sub['status'] = '';
				$sub['game_user_id'] = $sub['playerId'];
				$sub['game_user_account'] = $sub['pname'];
				$sub['game_user_nickname'] = $sub['prolename'];
				$sub['endTime'] = $sub['lockEndTime']==1?"--已手动解封": date('Y-m-d H:i:s',$sub['lockEndTime']);
				$sub['registerTime'] = date('Y-m-d H:i:s',$sub['registerTime']);
				$sub['URL_del'] = $this->_urlLockAccountDel($sub['playerId'],$sub['pname'],$sub['prolename']);
			}
