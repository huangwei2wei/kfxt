...
class ActionGame_MasterTools_LockAccount_SanFen extends Action_ActionBase{
	const INGAME = 'inGame';
	const LOGTYPE = 1;
...
// 	 print_r($data);
		$list =$data['data'][0]['players'];
		if(is_array($list)){
			foreach($list as $key => &$sub){
				$sub['game_user_id'] = $sub['playerId'];
				$sub['game_user_account'] = $sub['accountName'];
				$sub['game_user_nickname'] = $sub['playerName'];
// 				echo $sub['endTime']/1000;echo '----';
				$sub['endTime'] = $sub['endTime']?date('Y-m-d H:i:s',$sub['endTime']/1000):$sub['endTime'];
// 				$sub['startTime'] = date('Y-m-d H:i:s',$sub['startTime']);
				$sub['URL_del'] = $this->_urlLockAccountDel($sub['playerId'],$sub['accountName'],$sub['playerName']);
			}
			
			$this->_loadCore('Help_Page');//载入分页工具
			$helpPage=new Help_Page(array('total'=>intval($data['data'][1]['total']),'perpage'=>PAGE_SIZE));
