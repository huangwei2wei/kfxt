...
class ActionGame_MasterTools_LockAccount_HuanJL extends Action_ActionBase{
	const INGAME = 'inGame';
	const LOGTYPE = 1;
	...
	private function _dataInGame($UrlAppend=null,$get=null,$post=null){
		...
		if(is_array($list)){
			foreach($list as $key => &$sub){
				...
				$sub['endTime'] = date('Y-m-d H:i:s',$sub['endTime']);
				$sub['startTime'] = date('Y-m-d H:i:s',$sub['startTime']);
				$sub['URL_del'] = $this->_urlLockAccountDel($sub['UserID'],$sub['UserName'],$sub['NickName']);
			}
		}
