...
class ActionGame_MasterTools_LockAccount_ZhiDouXing extends Action_ActionBase{
	const INGAME = 'inGame';
	const LOGTYPE = 1;
	...
	public function main($UrlAppend=null,$get=null,$post=null){
		...
		if(is_array($data['data'])){
			foreach($data['data'] as $key => $sub){
				...
				$dataList[$key]['game_user_id'] = $sub['account'];
				$dataList[$key]['game_user_account'] = $sub['playerName'];
				$dataList[$key]['endTime'] = date('Y-m-d H:i:s',$endTime);
				$dataList[$key]['remark'] = $sub['remark'];
				
				if($sub['status']==0 ){
					$dataList[$key]['status'] ='强制解封';
				}elseif ($sub['status']==1 ){
					if(time() <$endTime && time()> $startTime){
						$dataList[$key]['status'] ='封号中';
					}elseif (time() < $startTime){
						$dataList[$key]['status'] ='封号尚未开始';
					}elseif (time() > $endTime ){
						$dataList[$key]['status'] ='自动解封';
					}
				}
				
				$dataList[$key]['URL_del'] = $this->_urlLockAccountDel($sub['account']);
			}
		}
