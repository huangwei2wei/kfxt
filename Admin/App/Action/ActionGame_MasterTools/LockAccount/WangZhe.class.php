...
class ActionGame_MasterTools_LockAccount_WangZhe extends Action_ActionBase{
	const INGAME = 'inGame';
	const LOGTYPE = 1;
	...
	private function _dataInGame($UrlAppend=null,$get=null,$post=null){
		...
		if(is_array($list)){
			foreach($list as $key => &$sub){
				...
				$sub['endTime'] = date('Y-m-d H:i:s',$sub['forbitEndTime']);
	...
	private function _urlLockAccountDel($playerId=null,$playerAcount=null,$playerNickName=null){
		$query = array(
			...
			'__game_id'=>$this->_gameObject->_gameId,
			'server_id'=>$_REQUEST['server_id'],
			'playerId'=>base64_encode($playerId),
