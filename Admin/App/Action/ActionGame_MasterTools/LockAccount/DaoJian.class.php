...
class ActionGame_MasterTools_LockAccount_DaoJian extends Action_ActionBase{
	const INGAME = 'inGame';
	const LOGTYPE = 1;
		...
		if(is_array($list)){
			foreach($list as $key => &$sub){
				if($sub['forbitLogin'] != null){
					...
					$dataList[$key]['endTime'] = date('Y-m-d H:i:s',round($sub['forbitLogin']/1000 ));
					$dataList[$key]['URL_del'] = $this->_urlLockAccountDel($sub['playerId'],$sub['userName'],$sub['playerName']);
				}
			}
		}
