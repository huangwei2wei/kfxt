...
class ActionGame_MasterTools_ItemCardApply2_GongFu extends Action_ActionBase{
	...
	public function getPostData($post=null){
		...
		$postData = compact('type','repeat','keyType','name','counts','bindType','playerId','goldValue','exchangeType','goldTickeValue','assetValue','mailTitle','mailContent','expiration','goods','conditions');
//		if($bindType == 1){
...
sendMailInfo;
		...
		if($postData['goldValue']>0){
			//记录元宝
			$itemsInfo .= $this->_assign[self::EXCHANGE_TYPE][$postData['exchangeType']]."(<font color='#FF0000'>{$postData['goldValue']}</font>)、";
		}
		...
		if($postData['goldTickeValue']>0){
			//记录银票
			$itemsInfo .= "银票(<font color='#FF0000'>{$postData['goldTickeValue']}</font>)、";
		}
		$itemsInfo .=$this->_getItemsData('ItemsInfo');	//记录道具内容
		$receiveConditions = $this->_getReceiveConditions('info');//领取条件
		...
		$applyInfo =<<<applyInfo
			...
			<div style="padding:3px; margin:3px; border:1px dashed #000">
				...
				{$bindPlayerIdInfo}
				{$sendMailInfo}
				<div>卡内物品：{$itemsInfo}</div>
				<div>领取条件：{$receiveConditions}</div>
			</div>
applyInfo;
