...
class ActionGame_OperatorTools_ShopProduce_SanFen extends Action_ActionBase{
...
// 		print_r($data);
		if($data['status'] == '1'){
			...
			foreach ($data['data'] as &$sub){
				$sub['hot'] = $ys[$sub['hot']];
				$sub['fashion'] = $ys[$sub['fashion']];
				$sub['horse'] = $ys[$sub['horse']];
				$sub['gift'] = $ys[$sub['gift']];
				$sub['vip'] = $ys[$sub['vip']];
				$sub['mystical'] = $ys[$sub['mystical']];
				$sub['purchase'] = $ys[$sub['purchase']];
				$sub['forbid'] = $ys[$sub['forbid']];
...
// 		$this->_assign['pageBox'] = $helpPage->show();
	...
	private function _urlEdit($id){
		...
		return Tools::url(CONTROL,'ShopProduceEdit',$query);
	}
