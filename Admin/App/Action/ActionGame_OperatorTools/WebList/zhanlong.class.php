...
class ActionGame_OperatorTools_WebList_zhanlong extends Action_ActionBase{
	...
	public function main($UrlAppend=NULL,$get=NULL,$post=NULL){
		...
		$page = ($page-1)*50;
		$data = $this->Select("select * from  t_web_list order by world_id asc limit ".$page.","."50",$this->jdbc,$this->user,$this->pwd);
		$dataCount = $this->Select("select count(*) from  t_web_list",$this->jdbc,$this->user,$this->pwd);
	...
	private function _urlDel(){
		...
		return Tools::url(CONTROL,'WebList',$query);
	}
	...
	private function _urlModify(){
		...
		return Tools::url(CONTROL,'WebAdd',$query);
	}
	...
	private function _del(){
		$sql = "delete from t_web_list where world_id='".$_GET["world_id"]."'";
		if($this->Select($sql,$this->jdbc,$this->user,$this->pwd,true)){
			$this->jump('操作成功',1);
		}else{
			$errorInfo = '操作失败:';
