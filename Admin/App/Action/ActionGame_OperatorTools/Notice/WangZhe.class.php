...
class ActionGame_OperatorTools_Notice_WangZhe extends Action_ActionBase{
...
// 		print_r($data);
		if($data['status'] == '1' && $data['data']['doingList']){
			foreach ($data['data']['doingList'] as &$sub){
				$sub['title']	=	strip_tags($sub['message']);
				$sub['URL_del'] 	= $this->_urlNoticeDel($sub['id']);
				$sub['URL_edit'] 	= $this->_urlNoticeEdit($sub['id']);
			}
			$this->_assign['dataList']=$data['data']['doingList'];
		}
