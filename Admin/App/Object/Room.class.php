...
class Object_Room extends Object implements Serializable {
	 ...
	 * @param int $num
	 */
	public function setupIncompleteOrder($order){
	 ...
	 * @param int $num
	 */
	public function completeOrder($num=-1){
		$this->_orderNum['incomplete']+=$num;
		if ($this->_orderNum['incomplete']<0)$this->_orderNum['incomplete']=0;
	}
	...
	public function kickAllUser() {
		...
		foreach ( $this->_userClassList as $userName ) {
			...
			if (method_exists($userClass,"outRoom")){
				$userClass->outRoom ();
			}
			if (method_exists($userClass,"setUpdateInfo")){
