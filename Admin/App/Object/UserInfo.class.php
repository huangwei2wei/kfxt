...
class Object_UserInfo extends Object implements Serializable {
	...
	protected $_act=array();	//用户权限
	protected $_moudleAct = array();//用户的额外 ‘模块’ 权限
	protected $_program=array();	//技术部参数
	 ...
	 * 返回用户能管理的运营商id
	 */
	public function getUserOperatorIds($gameId = 0){
		static $arr=array();	//静态 ,单一模式.
		if (!count($arr)){
			if($gameId){
				foreach ($this->_operatorIds as $value){
					if($value['game_type_id'] == $gameId){
	 ...
	 * @param int $num
	 */
	public function setupIncomplete($num){
		$this->_incompleteOrderNum=$num;
	}

	/**
	 * 获取当天/所有工单数
	 * @param int 日期 date('Ymd')
	 */
	...
	public function checkCurOperLv($operatorArr){
		...
		if (!count($this->_operatorIds))return false;	//如果没有设置运营商那么将直接退出
	...
	public function serialize() {
		...
		$data ['act']=$this->_act;
		$data ['moudle_act']=$this->_moudleAct;
	...
	public function unserialize($data) {
		...
		$this->_act=$data ['act'];
		$this->_moudleAct=$data ['moudle_act'];
	...
	private function _setBaseInfo(){
		...
		#------保存完整姓名------#
		$this->_program['position_id']=$userInfoArr['position_id'];
		$this->_program['project_id']=$userInfoArr['project_id'];
		...
		$this->_status=$userInfoArr['status'];
		if ($userInfoArr['order_vip_level']!='')
			$this->_orderVipLevel=explode(',',$userInfoArr['order_vip_level']);
		if ($userInfoArr['act'])$this->_act=explode(',',$userInfoArr['act']);
		if($userInfoArr['moudle_act']){
			$this->_moudleAct=unserialize($userInfoArr['moudle_act']);
		}
		$this->_updateMail();
	}
	...
	public function outRoom(){
		if ($this->_roomId!=0){//如果在房间
			$utilRooms=$this->_getGlobalData('Util_Rooms','object');
