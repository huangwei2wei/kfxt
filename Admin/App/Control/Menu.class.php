	...
	private function _index() {
		...
		foreach ( $dataList as &$value ) {
			...
			foreach ( $value ['actions'] as &$childValue ) {
				...
				$childValue ['word_status'] = $childValue ['status'] ? Tools::getLang('DISPLAY','Common') : Tools::getLang('NOT_DISPLAY','Common');
			}
			...
			$value ['word_status'] = $value ['status'] ? Tools::getLang('DISPLAY','Common') : Tools::getLang('NOT_DISPLAY','Common');
		}
		...
		if ($this->_isAjax()){
			if ($_POST['Id']){
				$updateArr=array(
					...
					'status'=>Tools::coerceInt($_POST['status']),
					'name'=>$_POST['name'],
					'name_2'=>$_POST['name_2']
				);
