<?php
class Control_BTO2Master extends BTO2 {
	...
	private function _createUrl(){
		...
		$this->_url['OperationFRG_Notice_serverSyn']=Tools::url('BTO2Operation','Notice',array('zp'=>self::PACKAGE,'doaction'=>'serverSyn'));
		...
		$this->_url['OperationFRG_RewardServerSyn']=Tools::url('BTO2Operation','Reward',array('zp'=>self::PACKAGE,'doaction'=>'serversyn'));
		...
		$this->_url['OperationFRG_Libao_syncard']=Tools::url('OperationFRG','Libao',array('zp'=>self::PACKAGE,'doaction'=>'syn_card'));
		$this->_url['OperationFRG_Libao_serverSyn']=Tools::url('OperationFRG','Libao',array('zp'=>self::PACKAGE,'doaction'=>'serverSyn'));
	 ...
	 */
	public function actionShowtool(){
	...
	public function actionEventList(){
		...
		if ($_REQUEST['server_id']){
			...
			if(isset($theData) ){
				...
				$this->_loadCore('Help_Page');
				$helpPage = new Help_Page ( array ('total' => $theData['TotalNum'], 'perpage' => ($theData['PageSize'] )) );
		...
		$this->_view->assign('server_id',$_REQUEST['server_id']);
		$this->_utilMsg->createPackageNavBar();
		$this->_view->display();
	}
...
//						$this->_modelFrgLog->add($_POST,1);
	...
	private function _donttalkIndex(){
		...
		if ($_REQUEST['server_id']){
			...
			$data=$this->_utilFRGInterface->callInterface();
			if ($data){
				if ($data['data']['UserLockSay']){
					foreach ($data['data']['UserLockSay'] as $key=>&$value){
						if(is_array($data['data']['UserData'][$key])){
					...
					$this->_view->assign('dataList',$data['data']['UserLockSay']);
				}
			}else {
				$this->_view->assign('errorConn',Tools::getLang('CONNECT_SERVER_ERROR','Common'));
			}
	...
	private function _rewardIndex(){
		...
		$this->_view->set_tpl(array('body'=>'BTO2/BTO2Master/Reward.html'));
		$this->_utilMsg->createPackageNavBar();
		$this->_view->display();
	}
...
//				echo $data['User']->Properties->VUserName;
				...
				$this->_view->assign('UserName',$data['data']['UserName']);
			}else {
				$this->_view->assign('errorConn',Tools::getLang('CONNECT_SERVER_ERROR','Common'));
			}
		}
...
//			$this->_view->assign('operatorList',$operatorList);
			...
			$this->_view->set_tpl(array('body'=>self::PACKAGE.'/'.self::OPT.'/PayLibaoServerSyn.html'));
			$this->_utilMsg->createPackageNavBar();
			$this->_view->display();
		}
	}
...
//							$this->_modelFrgLog->add($_POST,5);
	...
	public function actionGameLogin(){
		if ($this->_isPost()){
			...
			$siteurl='1';
			$apiUserId='1';
			$gmSign=md5("Uname={$userName}&userid={$apiUserId}&GameId={$gameId}&ServerId={$serverid}&Key={$sysKey}&Time={$time}&al={$al}&from={$from}&siteurl={$siteurl}&job=1");
	...
	if($this->_isPost()){
		...
		$post	=	array(
			...
			//"MsgTitle"		=>	NULL,
			//"MsgContent"	=>	NULL,
			"ToolId"		=>	explode(",",$_POST["delid"]),
			"ToolNum"		=>	explode(",",$_POST["delItems"]),
