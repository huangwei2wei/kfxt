<?php
abstract class HaiDao extends Control {
	...
	const GAME_ID=8;	
	const PACKAGE='HaiDao';
	const APPLY_TPYE_GOLD = 4;
	const MASTER = 'HaiDaoMaster';	
	const OPT = 'HaiDaoOperation';	
	const GOLD = 'HaiDaoGold';
...
//		$operatorList=Model::getTtwoArrConvertOneArr($operatorList,'Id','operator_name');
	 ...
	 * 建立服务器选择列表
	 */
	protected function _createallServerList(){
		$gameServerList=$this->_getGlobalData('server/server_list_'.self::GAME_ID);
	...
	protected function getItemsFromOneServer($time = 86400){
		...
		if(false === $data){			
			...
			if($count && is_array($gameser_list)){
				...
				$serverId = is_array($_REQUEST['server_id'])?0:intval($_REQUEST['server_id']);
				if($serverId){
					$_msg = $gameser_list[$serverId];
