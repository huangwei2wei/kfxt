...
class Log_LockAccount extends Log_LogBase{
...
//		);
		...
		if($subject->_silenceDel){
			$AddLog = array(
				array('操作','<font style="color:#F00">解除封号</font>'),
		...
		foreach($subject->_playersData as $sub){
			...
			$GameOperateLog = $modelGameOperateLog->MakeDataForStore($userInfo,1,$subject->_serverId,$AddLog);
