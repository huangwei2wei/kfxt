...
//官网所属ID 
			...
			$servers = $dao->where("game_type_id={$gameId} and operator_id={$filter['operator_id']} and marking !='' and server_url !='' ")->select();
			if(in_array($gameId, array(3))){
				$servers = $dao->where("game_type_id={$gameId} and marking !='' and server_url !='' ")->select();
			}
...
//			}
	...
	private function _getCreateUserServerIds($gameId){
		...
		if (!$serverList){
			...
			foreach ($serverList as $k=>$list){
				if (strpos($list,'N'))unset($serverList[$k]);
			}
			if ($serverList){
