 ...
 */
 class FaqAction extends BaseAction{
 	...
 	public function _initialize(){
 		parent::_initialize();
 		 //游戏列表
 		$this->_faqKind=D('QuestionTypes')->getGameList();//$this->getSysConfig("game_type");
// 		if (!isset($_GET['game_type_id']))$_GET['game_type_id']=2;
 		$this->assign('gameTypeId',$_GET['game_type_id']);
 		$_GET['game_type_id'] = $this->getGameId();
...
//		$gameList = D('PlayerFaq')->getGameList();
//		$this->assign('gameList',$gameList);
		$gameId = $this->getGameId();
		$kind_id=intval($_GET['typeid']);
		$map=array();
		if($kind_id)$map['kind_id']=$kind_id;
		$map['game_type_id']=$gameId;
		$map['lang_id']= $_GET['lang_id']?$_GET['lang_id']:C('LANG_ID');	//简体
		...
		//FAQ分类
		$types=$this->_getFaqTypes($gameId,$map['lang_id']);
	...
	public function ls(){
		...
		//FAQ分类
		$types=$this->_getFaqTypes($_GET['game_type_id'],$map['lang_id']);
		$this->assign("types",$types);
		$this->assign('keywords',$this->_getFaqKeywords($_GET['game_type_id']));
		$this->assign('faqKind',$this->_faqKind);
		$this->display("ls");
	 ...
	 */
	private function _getFaqTypes($gameTypeId,$lang_id){
		$faqKindKey="player_kind_faq_{$gameTypeId}";//faq分类缓存
		$types=S($faqKindKey);
		if (!$types){
			$faqTypesDao = M("player_kind_faq");
			$lang_id = $lang_id?$lang_id:C('LANG_ID');
			$types=$faqTypesDao->where("game_type_id={$gameTypeId} and lang_id=".$lang_id)->order('sort_id asc')->select();
			foreach ($types as &$v){
				$value['ask_url'] = U('Faq/index',array('game_type_id'=>$value['Id'],'typeid'=>$v['Id'],'lang_id'=>$lang_id));
			}
	...
	public function show(){
		...
		//FAQ分类
		$types=$this->_getFaqTypes($_GET['game_type_id'],$_GET['lang_id']);
		$this->assign("types",$types);
		$this->assign('keywords',$this->_getFaqKeywords($_GET['game_type_id']));
		$this->assign('faqKind',$this->_faqKind);
		$this->display("show");
	...
	public function search(){
		...
		if(isset($keyword)){
			...
			$this->assign("types",$this->_getFaqTypes($game_id),$_GET['lang_id']);
			$this->assign('keywords',$this->_getFaqKeywords($_GET['game_type_id']));
	...
	public function bankList(){
		...
		//FAQ分类
		$types=$this->_getFaqTypes(C('FAQ_BANK_ID'),$_GET['lang_id']);
		$this->assign("types",$types);
	...
	public function bankShow(){
 	...
 	public function questionTop(){
 		$topFaqlist = D('PlayerFaq')->getTopFAQ($_GET['gametype']);
